...
const SignIn = ({isOpen, setIsOpen}) => {
    ...
    let modalErrMsgDetails = "Sorry, we can’t find an account with this email address, please try again or create a new account.";

    useEffect(() => {
        document.title = "Sign In - InformHer";
    }, []);
        ...
        <div  ref={signInRef} className='bg-[#1D2123] text-white flex flex-col min-h-screen'  style={{display: 'block'}}>
            ...
            <div className='w-full bg-inherit flex items-center mx-auto justify-center'>
                <div className='text-left font-Poppins pl-4 lg:pl-0 flex flex-col'>
                    <h4 className='text-[#95B4B3] font-extrabold text-[1.5rem] leading-[2.25rem] text-left'>Sign in</h4>
                    <p className='text-[#FACD66] font-bold leading-[1.6875rem] text-lg'>Welcome back to VizMuz</p>
                    <form onSubmit={handleSubmit} className='flex flex-col text-[#000]'>
                        <label  className='mt-6 text-[#95B4B3] text-base mb-2'>Email</label>
                        <input id='emailInput' value={email} onChange={(e) => setEmail(e.target.value)} placeholder='Enter your email' className='focus:border-0 focus:border-[#95B4B3] border-2 px-5 py-3 rounded-md border-[#95B4B3] w-[18rem] lg:w-[19.6875rem] h-[2.8125rem] mb-5 placeholder:text-sm md:placeholder:text-base placeholder:text-[rgba(119, 114, 211, 0.5);]'></input>
                        <label htmlFor='passwordInput' className='text-[#95B4B3] text-base mb-2'>Password</label>
                        <div className='flex w-[18rem] lg:w-[19.6875rem]'>
                            <input type={password.showPassword ? "text" : "password"}
                                ...
                                className='focus:border-0 focus:border-[#95B4B3] border-2 border-r-0 w-3/4 px-5 py-3 rounded-md rounded-tr-none rounded-br-none border-[#95B4B3] h-[2.8125rem] placeholder:text-sm md:placeholder:text-base placeholder:text-[rgba(119, 114, 211, 0.5)'></input>
                            <button 
                                onClick={handleClickShowPassword} 
                                onMouseDown={handleMouseDownPassword}
                                className='w-1/4 border-2 border-l-0 rounded-md border-[#95B4B3] rounded-bl-none rounded-tl-none'><FaEye className='mx-auto'/></button>
                        </div>
                        <div className='flex justify-between px-2 lg:px-0 mt-4 text-[#95B4B3] text-sm'>
                            <p className='cursor-pointer'><input type="checkbox" id="rememberMe" ref={rememberMeRef}></input> <label htmlFor="rememberMe">Remember me?</label></p>
                            <p className='cursor-pointer'><Link to='/forgot-password'>Forgot Password</Link></p>
                        </div>

                        <button onClick={handleButtonClick} type="submit" className='bg-[#95B4B3] rounded-md my-5 px-8 lg:px-12 py-3 md:py-4 w-[18rem] lg:w-[19.6875rem] text-white'>
                            Sign in
                        </button>
