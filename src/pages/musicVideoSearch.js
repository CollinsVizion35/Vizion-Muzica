...
const MusicVideoSearch = () => {
    ...
    const searchVideos = async (e) => {
        e.preventDefault()
        const {data} = await axios.get("https://youtube138.p.rapidapi.com/search/", {
            headers: {
    ...
    return (
        <>
            <div className='bg-[#1D2123] text-white'>
                ...
                <div>
                {videos.length > 0 ? videos.map((video, watch) => {
                        return (
                            <> 
                            
                            
                                <div key={watch} className='w-[90vw] my-2'>
                                    <iframe className='w-[100%] h-[50vh]' src={`https://www.youtube.com/embed/${video.video.videoId}`} title={video.video.title}></iframe>
