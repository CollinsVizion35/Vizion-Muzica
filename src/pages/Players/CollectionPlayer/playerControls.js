...
function PlayerControls(props) {
  ...
  const {
    ...
    const onChange = (e) => {
      const audio = playerAudio2Ref.current
  ...
  return (
    <div className='flex flex-col items-center w-[60vw]'>
        <div className='flex flex-row items-center w-[40vw] lg:justify-between ml-[50%] lg:ml-[0px] mb-2'>
            <button className={shuffle? 'hidden lg:block' : 'hidden lg:block text-[#F9D175]'} onClick={() => {setShuffle(!shuffle); console.log(shuffle)}}><BsShuffle/></button>
            ...
            <button className={repeat? 'hidden lg:block' : 'hidden lg:block text-[#F9D175]'} onClick={() => {setRepeat(!repeat); console.log(repeat); console.log(collections.length)}}><TbRepeatOnce/></button>
        </div>
